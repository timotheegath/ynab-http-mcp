...
def test_tools_use_simplified_validation():
    ...
    try:
        ...
        with open(
            "/home/timo/projects/ynab-http-mcp/src/ynab_http_mcp/tools/transactions.py",
            "r",
        ) as f:
            transaction_content = f.read()
            assert "clean_ynab_data" in transaction_content, (
                "Transaction tool should use clean_ynab_data"
            )
            assert "simple_validate" in transaction_content, (
                "Transaction tool should use simple_validate"
            )
            assert "validate_and_clean_data" not in transaction_content, (
                "Transaction tool should not use old validation"
            )

        # Check category tool imports
        with open(
            "/home/timo/projects/ynab-http-mcp/src/ynab_http_mcp/tools/categories.py",
            "r",
        ) as f:
            category_content = f.read()
            assert "clean_ynab_data" in category_content, (
                "Category tool should use clean_ynab_data"
            )
            assert "simple_validate" in category_content, (
                "Category tool should use simple_validate"
            )
            assert "validate_and_clean_data" not in category_content, (
                "Category tool should not use old validation"
            )

        # Check planning tool imports
        with open(
            "/home/timo/projects/ynab-http-mcp/src/ynab_http_mcp/tools/planning.py", "r"
        ) as f:
            planning_content = f.read()
            assert "simple_validate" in planning_content, (
                "Planning tool should use simple_validate"
            )
            assert "validate_and_clean_data" not in planning_content, (
                "Planning tool should not use old validation"
            )
...
def test_simplified_schemas_exist():
    ...
    try:
        ...
        for schema_name in expected_schemas:
            assert schema_name in registered_schemas, (
                f"{schema_name} should be registered"
            )
