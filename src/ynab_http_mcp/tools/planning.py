...
def register(mcp, ynab_service: YnabService):
    ...
    async def get_plan_month(
        month_date : Annotated[str | None, "ISO-format date within the month of choice. For instance, '2023-12-11' targets December 2023. Leave blank to select current month"] = None,
    ) -> dict[str, Any]:
        ...
        if month_date:
            try:
                ...
                raise RuntimeError(f"Failed to convert input date to datetime: {e}") from e
        else:
