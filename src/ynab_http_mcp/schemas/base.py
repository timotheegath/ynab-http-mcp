...
class MCPValidationError(Exception):
    ...
    def __init__(
        self, model_name: str, raw_data: Any, validation_error: ValidationError
    ):
        ...
        for error in validation_error.errors():
            ...
            error_details.append(
                f"{field_path}: {error['msg']} ({error.get('type', 'unknown')})"
            )

        super().__init__(
            f"Validation failed for {model_name}: " + "; ".join(error_details)
        )


def validate_and_clean_data(
    model: Type[T], raw_data: Any, debug_mode: bool = False, **validation_kwargs
) -> T:
    """
    ...
    try:
        ...
        raise MCPValidationError(
            model.__name__, raw_data, ValidationError([], [])
        ) from e
...
class CleanBaseModel(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "examples": [{"description": "Example cleaned YNAB data", "value": {}}]
        }


def create_response_model(
    data_model: Type[BaseModel], response_name: str = "Response"
) -> Type[BaseModel]:
    """
    ...
    class ResponseModel(BaseModel):
        data: List[BaseModel] = Field(
            ..., description=f"List of {data_model.__name__} items"
        )
